...
if [ -d "$LEGACY_ROOT" ]; then
  ...
  if [ -n "$MIGRATED_MODELS" ]; then
    for model in $MIGRATED_MODELS; do
      ...
      if curl -fsS -X POST "http://127.0.0.1:11435/api/pull" -H "Content-Type: application/json" -d "$payload" >/dev/null; then
        echo "Migrated $model"
      else
        echo "Could not auto-migrate $model (may require manual setup)."
      fi
    done
  else
  ...
  echo "No legacy Ollama manifest directory found; skipping migration step."
fi
...
for model in $REQUIRED_MODELS; do
  ...
  if curl -fsS -X POST "http://127.0.0.1:11435/api/pull" -H "Content-Type: application/json" -d "$payload" >/dev/null; then
    ...
    echo "Failed to ensure $model. Please add it manually."
  fi
done
